...
public class TrailRendererWith2DCollider : MonoBehaviour
{
    ...
    private Vector3 nowposition;    // = trans.position + offset
    ...
    private void Awake()
    {
        ...
        //collider.SetPath(0, (List<Vector2>)null);
        ...
        centerPositions.AddFirst(nowposition);
    ...
    private void Update()
    {
        nowposition = trans.position + offset;
        if (!pausing)
        {
    ...
    private bool TryAddVertices()
    {
        ...
        if ((centerPositions.First.Value - trans.position).sqrMagnitude > vertexDistanceMin * vertexDistanceMin)
        {
            ...
            leftVertices.AddFirst(new Vertex(leftPos+offset, trans.position, (leftPos - trans.position).normalized));
            rightVertices.AddFirst(new Vertex(rightPos+offset, trans.position, (rightPos - trans.position).normalized));

            //Instantiate(test, new Vector3((leftPos.x+rightPos.x)/2f, (leftPos.y+rightPos.y)/2f, 1f), Quaternion.identity);
            //Instantiate(test, new Vector3(leftPos.x, leftPos.y, 1f), Quaternion.identity);
            //Instantiate(test, new Vector3(rightPos.x, rightPos.y, 1f), Quaternion.identity);
        ...
        */
        if ((centerPositions.First.Value - nowposition).sqrMagnitude > vertexDistanceMin * vertexDistanceMin)
        {
            //calculate the normalized direction from the 1) most recent position of vertex creation to the 2) current position
            Vector3 dirToCurrentPos = (nowposition - centerPositions.First.Value).normalized;

            //calculate the positions of the left and right vertices --> they are perpendicular to 'dirToCurrentPos' and 'renderDirection'
            Vector3 cross = Vector3.Cross(renderDirection, dirToCurrentPos);
            Vector3 leftPos = nowposition + (cross * -widthStart * 0.5f);
            Vector3 rightPos = nowposition + (cross * widthStart * 0.5f);

            //create two new vertices at the calculated positions
            leftVertices.AddFirst(new Vertex(leftPos + offset, nowposition, (leftPos - nowposition).normalized));
            rightVertices.AddFirst(new Vertex(rightPos + offset, nowposition, (rightPos - nowposition).normalized));
